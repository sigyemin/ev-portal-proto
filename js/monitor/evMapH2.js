...
var Station = function( sid, snm, cst, cstnm, pos, posnm, adr, x, y, zscode, fee, call ) {
	this.sid = sid;
	this.snm = snm;
	this.adr = adr;
	this.x = x;
	this.y = y;
	this.stat = cst; //대표상태 : ( 0:알수없음, 1:통신이상, 2:충전대기, 3:충전중, 4:운영중지, 5:점검중 ) 
	this.statnm = cstnm; 
	this.pos = pos; 
	this.posnm = posnm; 
//	this.utime  = utime; //이용시간정보 "24시간" 24시간이용 가능 
	...
	this.setStat = function( cst ) {
		this.stat = cst;
	};
	
	this.setStatNm = function( cstnm ) {
		this.statnm = cstnm;
	};
...
function reDrawStatAndMarker(){
	...
	setTimeout(function () { this.dispatchEvent(new CustomEvent("custom_drawMarker"), false, false) }, 50);	// 202306274 : 추가
}
	...
	for( var i = 0 ; i < tKeys.length ; i++ ) {
		...
		if(theStation.stat != ""){
			...
			stationSpot[arrayNum] = { 
					sid: theStation.sid
					, id: arrayNum
					, y: parseFloat(theStation.y)
					, x: parseFloat(theStation.x)
		...
		$.each(stationSpot, function (key, object) {
			if (idkey == object.id) {
				...
				stationFeature[i] = new ol.Feature({
                    ...
                    ,sid: object.sid ,id: object.id, stationY: object.y, stationX: object.x, stat: object.stat
                });
			}
		});
	});
	
	stationLayer = new ol.layer.Vector({ name : 'station', id : 'station', source: new ol.source.Vector({features: stationFeature}), style: createStyle, zIndex: 40 });
...
function getMarkerIcon( stat ) {
	// var oIcon, iConSize, imgUrl;
	var imgUrl;
	
	imgUrl = getImageInfo(stat);
	
	return imgUrl;
};

function getImageInfo(stat_cd) {
	if (stat_cd == "") stat_cd = "0";
	
	if (stat_cd == 6) stat_cd = 1;
	else if (stat_cd == 7) stat_cd = 9;
	else if (stat_cd == 5) stat_cd = 1;

	var imgAlt = '수소충전소';
//	imgUrl = '/portal/_images/marker/m_89_' + stat_cd + '.png';
//	imgUrl = '../img/monitor/marker/m_89_' + stat_cd + '.png';
	var imgUrl = $("div#map").data("contextpath") + '/img/monitor/marker/m_89_' + stat_cd + '.png';
	
	return imgUrl;
}

function getMarkerKey( stat_cd ) {
	
	var markerKey = new Array;

	if (stat_cd == "30") stat_cd = "2";
	else if (stat_cd == "20") stat_cd = "3";
	else if (stat_cd == "10") stat_cd = "1";
	else stat_cd = "0";
...
function getStatSpan_OLD(stat) {	// TODO : DELETEME
	var span = "";
	
	if(stat == 2 || stat == '2' ){//충전기대기
		span = "<span class='condition03'>사용가능</span>";
...
function getStatSpan(stat, statnm, pos, posnm) {
	if(stat == 10 || stat == '10' ){      // 10 : 영업정지
		return '<span class="state state_black">' + statnm + '</span>';
	}else if(stat == 20 || stat == '20' ){// 20 : 영업마감
		return '<span class="state state_org">' + statnm + '</span>';
	}else if(stat == 30 || stat == '30' ){// 30 : 운영중
		if (pos == 0 || pos == '0' ) { statnm = posnm; }
		return '<span class="state">' + statnm + '</span>';
	}else
...
// 20231026 : 전화번호 양식변경
function formatPhone(call) {
	var cleanCall = call.replaceAll(/[^\d]/, '');
	if (cleanCall.length <= 1) return call;
	else if (cleanCall.length == 8) {
		call = cleanCall.slice(0,4)+"-"+cleanCall.slice(4,8);
	} else if (cleanCall.length == 9) {
		call = cleanCall.slice(0,2)+"-"+cleanCall.slice(2,5)+"-"+cleanCall.slice(5);
	} else if (cleanCall.length == 10) {
		if ( cleanCall.slice(0,2) == "02" ) {
			call = cleanCall.slice(0,2)+"-"+cleanCall.slice(2,6)+"-"+cleanCall.slice(6);
		} else {
			call = cleanCall.slice(0,3)+"-"+cleanCall.slice(3,6)+"-"+cleanCall.slice(6);
		}
	} else if (cleanCall.length == 11) {
		call = cleanCall.slice(0,3)+"-"+cleanCall.slice(3,7)+"-"+cleanCall.slice(7);
	} else if (cleanCall.length == 12) {
		call = cleanCall.slice(0,4)+"-"+cleanCall.slice(4,8)+"-"+cleanCall.slice(8);
	}
