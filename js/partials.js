...
(function () {
                ...
                <div class="mega-columns mega-columns-5">
                  ...
                  <div class="mega-col">
                    ...
                    <ul>
                      ...
                      <li><a href="charging-card.html?tab=history" data-i18n="mega.charging.card.history">이용요금·명세서 조회</a></li>
              ...
              <div class="mega-inner">
                ...
                <div class="mega-columns">
                  <div class="mega-col">
                    ...
                    <ul>
                      <li><a href="data.html#tindex" data-i18n="mega.data.tindex">충전사업자 투명성지수</a></li>
  ...
  const sitemapSections = [
    ...
    { tKey: 'nav.charging', cols: [
      ...
      { tKey: 'mega.charging.card', items: [['내 회원카드','charging-card.html?tab=mycard','mega.charging.card.list'],['이용요금·명세서 조회','charging-card.html?tab=history','mega.charging.card.history'],['충전 포인트','charging-card.html?tab=point','mega.charging.card.point']] },
    ...
    { tKey: 'nav.data', cols: [
      { tKey: 'mega.data.dashboard', items: [['충전사업자 투명성지수','data.html#tindex','mega.data.tindex'],['무공해차 보급 현황','data.html#supply','mega.data.supply'],['충전 인프라 현황','data.html#infra','mega.data.infra'],['보조금 집행 현황','data.html#subsidy','mega.data.subsidy'],['K-EV100 전환 현황','data.html#k100','mega.data.k100']] },
  ...
  // ===== 마이페이지 좌측 사이드 네비 주입 =====
  const mpSideSlot = document.getElementById('mp-side-slot');
  if (mpSideSlot) { mpSideSlot.remove(); }   // 좌측 사이드 네비 폐지 — 슬롯 제거(본문 풀폭). 아래 주입 블록 비활성화.
  if (false) {
    const activeMp = window.__activeMp || 'home';
    const userName = (_loggedUser && _loggedUser.name) || '홍길동';
    const userInitial = userName.charAt(0);
    const userRoleKey = (_loggedUser && _loggedUser.kind === 'business') ? 'mp.side.role.business' : 'mp.side.role.personal';
    const userRoleFallback = (_loggedUser && _loggedUser.kind === 'business') ? '사업자 회원' : '일반 회원';
    ...
    const ICONS = {
      ...
      info:    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>',
      pw:      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>',
      bell:    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>',
      ...
      logout:  '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/></svg>',
      card:    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="2" y="6" width="20" height="13" rx="2"/></svg>',
      star:    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polygon points="12 2 15 8.5 22 9.3 17 14 18.5 21 12 17.5 5.5 21 7 14 2 9.3 9 8.5 12 2"/></svg>',
      ...
      check:   '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg>',
    ...
    // V3.2: i18n 키 추가 — [key, icon, href, fallback_label, i18n_key]
    const groups = [
      { titleKey: '', titleFallback: '', items: [
        ['home', 'home', 'mypage.html', '마이페이지 홈', 'mp.side.item.home']
      ]},
      { titleKey: 'mp.side.group.info', titleFallback: '내 정보 관리', items: [
        ['info',          'info',   'mypage-info.html',          '회원 정보 조회·수정',    'mp.side.item.info'],
        ['password',      'pw',     'mypage-password.html',      '비밀번호 변경',          'mp.side.item.password'],
        ['notifications', 'bell',   'mypage-notifications.html', '알림 설정',              'mp.side.item.notifications'],
        ['accounts',      'link',   'mypage-accounts.html',      '계정 연결',     'mp.side.item.accounts'],
        ['withdraw',      'logout', 'mypage-withdraw.html',      '회원 탈퇴',              'mp.side.item.withdraw']
      ]},
      ...
      { titleKey: 'mp.side.group.subsidy', titleFallback: '보조금·민원', items: [
        ...
        ['inquiry-status', 'check', 'inquiry-status.html',   '민원 처리 현황',   'mp.side.item.inquiry'],
        ['public-status',  'cal',   'mypage-contests.html',  '공모 신청 현황',   'mp.side.item.public']
      ]}
    ];

    let html = '<aside class="mp-side" data-i18n-aria="mp.side.aria" aria-label="마이페이지 메뉴">';
    html += '<div class="mp-side-profile">';
    html += '<span class="mp-side-avatar" aria-hidden="true">' + userInitial + '</span>';
    html += '<div><div class="mp-side-name">' + userName + '<span style="color:var(--text-tertiary);font-weight:500;font-size:12px;" data-i18n="util.user.suffix">님</span></div>';
    html += '<div class="mp-side-role" data-i18n="' + userRoleKey + '">' + userRoleFallback + '</div></div>';
    html += '</div>';

    groups.forEach(g => {
      html += '<div class="mp-nav-group">';
      if (g.titleFallback) html += '<div class="mp-nav-group-title" data-i18n="' + g.titleKey + '">' + g.titleFallback + '</div>';
      html += '<ul class="mp-nav-list">';
      g.items.forEach(it => {
        const k = it[0], ic = it[1], href = it[2], label = it[3], i18nKey = it[4];
        const isActive = (k === activeMp);
        const cls = isActive ? ' class="mp-nav-item active"' : ' class="mp-nav-item"';
        const ariaCur = isActive ? ' aria-current="page"' : '';
        if (href === '#notify') {
          html += '<li class="mp-nav-item"><a href="#" data-action="notify" data-action-label="' + label + '은(는) 준비 중입니다.">' + ICONS[ic] + '<span data-i18n="' + i18nKey + '">' + label + '</span></a></li>';
        } else {
          html += '<li' + cls + '><a href="' + href + '"' + ariaCur + '>' + ICONS[ic] + '<span data-i18n="' + i18nKey + '">' + label + '</span></a></li>';
        }
      });
      html += '</ul></div>';
    });
    html += '</aside>';
    mpSideSlot.outerHTML = html;
  }
